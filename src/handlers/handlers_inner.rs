...
pub async fn create_question(
    ...
    match question {
        ...
        Err(err) => {
            error!("Error while creating a question: {}", err);
...
pub async fn read_questions(
    ...
    match questions {
        ...
        Err(err) => {
            error!("Error while reading the questions: {}", err);
            return Err(HandlerError::default_internal_error());
        }
    }
}
...
pub async fn create_answer(
    ...
    match answer {
        ...
        Err(err) => {
            error!("Error while creating an answer: {}", err);

            match err {
                DBError::InvalidUUID(s) => return Err(HandlerError::BadRequest(s)),

                _ => return Err(HandlerError::default_internal_error()),
            }
        }
    }
}
...
pub async fn read_answers(
    ...
    match answers {
        Ok(answers) => return Ok(answers), // return answers
        Err(e) => {
            error!("Error while reading the answers: {}", e);
            return Err(HandlerError::default_internal_error());
        }
    }
}
